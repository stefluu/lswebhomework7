...
function contains(arr, item) {
  ...
  if(arr.includes(item) === true){ //tests if something is included in the array
	return true;
  }
  else{
	return false;
...
function averageTestScore(testScores) {
  ...
  //return the average
  var total = 0;
  var arrElements = testScores.length;
  for(var i = 0; i < testScores.length; i++){
    total += testScores[i];
  }
  return total/arrElements;
}
...
function makeCat(name, age) {
  ...
  //return the object
  var NewCat = function (name, age){
    this.name = name;
    this.age = age;
  };
  
  var cat = new NewCat(name, age);
...
function sumUserPostLikes(user) {
  ...
  //return the sum
  var sum = 0;
  var postsArr = user['posts'];
  for (var i = 0; i < postsArr.length; i++){
	sum += postsArr[i].likes;
  }  
  return sum;
}
